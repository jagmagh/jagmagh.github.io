...
| **Blog post headings: `###` / `####` / `#####`** (h3/h4/h5) — never `##` | The theme applies Montserrat uppercase bold to every heading level. At `##` (h2) section headings appear as large as the post title, making the page feel top-heavy. Dropping to h3 for main sections produces the right visual weight. Convention: `###` for top-level sections, `####` for sub-sections and sub-sub-sections. Do not use `#####` — Bootstrap 3 renders h5 at 14px, which is smaller than the 20px body text. Add `<br>` before each `###` heading for breathing room. Applied to all posts. |
